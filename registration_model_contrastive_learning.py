...
class RegistrationModel(BaseModel):
    ...
    def set_input(self, input):
        ...
        if len(self.gpu_ids) > 0:
            ...
            moving = moving.cuda(self.gpu_ids[0])
            fixed = fixed.cuda(self.gpu_ids[0])
        self.moving = moving
        self.fixed = fixed
        ...
        if len(self.gpu_ids) > 0:
            moving_atlas = moving_atlas.cuda(self.gpu_ids[0])
            fixed_atlas = fixed_atlas.cuda(self.gpu_ids[0])
